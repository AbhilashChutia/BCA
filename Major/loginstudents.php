...
if (isset($_POST['send'])) {
    ...
    $result = mysqli_query($connection, "SELECT * FROM bcamarks WHERE roll = '$roll' && password = '$password'");
    $row = mysqli_fetch_assoc($result);
    if (mysqli_num_rows($result) > 0) {
        if ($password == $row["password"]) {
            $_SESSION["login"] = true;
            $_SESSION["id"] = $row["id"];
            header("Location: outputstudents.php");
        } else {
...
<body>
    ...
    <section class="login">
        ...
        <form name="formstudents" onsubmit=" return validateFormstudents()" action="" method="post" class="">
            <div class="flex">
                <div class="inputBox">
                    <span>roll :</span>
                    <input type="number" name="roll" placeholder="Enter your roll" autocomplete="off">
                </div>
                <div class="inputBox">
                    <span>password :</span>
